<template>
	<view class='issue'>
		<view class="issue-head">
			...
			<image v-if="headPicShow" :src="headPicValue" class="issue-head-pic" mode="aspectFill"></image>
			...
			<view class="issue-head-star-box" v-if="starsShow">
				<view class="iconfont icon-star-fill" v-for="(item,index) in starsMax" :key="index" :class="{active:index<formatScore}" @click="setScore(index+1)"></view>
...
<script>
	export default {
		props:{
			...
			headPicValue:{
				type:String,
				default:require('@/static/feedback/avatar.png')
			},
			...
			/* starDefault:{ //未选中
				type:String,
				default:require('@/static/feedback/star.png'),
			},
			starActive:{
				type:String,
				default:require('@/static/feedback/star-active.png'),
			}, */
			score:{  //默认分数
				type:[Number,String],
				default:0
